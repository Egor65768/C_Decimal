...
Мною Decimal был реализован в виде четырехэлементного массива 32-разрядных целых чисел со знаком (`int bits[4];`).
...
*Если полученное число не вмещается в мантису при арифметических операциях, число округляется с помощью банковского округления.
Ps. Если округляемое число равноудалено от соседних чисел, то оно округляется до ближайшей чётной цифры заданного десятичного разряда. К примеру, 3,5 округляется до 4, а 2,5 до 2.
...
*При преобразовании числа типа float:
- *Если числа находятся в промежутке от 0 до 1e-28, вернуть ошибку и значение, равное 0*
- *Если числа больше 79,228,162,514,264,337,593,543,950,335 или равны бесконечности, вернуть ошибку*
...
*При преобразовании decimal в int, если у decimal есть дробная часть то она отбрасывается (напрмер 0.6387 преобразуется в 0)
